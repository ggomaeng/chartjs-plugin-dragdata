...
chains.forEach((chain) => {
  const BOND_CONTRACT =
    V2_BOND_CONTRACTS[chain] ?? "0xc5a076cad94176c2996B32d8466Be1cE757FAa27";
  module.exports[chain] = {
    tvl: async (api) => {
      const tokens = await api.fetchList({
        lengthAbi: "tokenCount",
        itemAbi: "tokens",
      ...
      const rTokens = (
        await api.multiCall({
          ...
          itemAbi: "tokens",
          target: BOND_CONTRACT,
          abi: "function tokenBond(address) view returns (address,  uint16, uint16,  uint40,  address reserveToken, uint256)",
        })
      ).map((i) => i.reserveToken);
      return sumTokens2({
        api,
        owner: BOND_CONTRACT,
        tokens: rTokens,
        permitFailure: true,
        blacklistedTokens: ownTokens[chain],
      });
    },
  };
  if (ownTokens[chain])
